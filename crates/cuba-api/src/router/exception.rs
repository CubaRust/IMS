...
async fn list_defects(
    ...
    Ok(AppJson(DefectService::new(state.db().clone()).list(&q).await?))
}
...
async fn get_defect(
    ...
    ctx.require_permission("defect.view")?;
    Ok(AppJson(DefectService::new(state.db().clone()).get(id).await?))
}
...
async fn list_recoveries(
    ...
    Ok(AppJson(RecoveryService::new(state.db().clone()).list(&q).await?))
}
...
async fn get_recovery(
    ...
    ctx.require_permission("recovery.view")?;
    Ok(AppJson(RecoveryService::new(state.db().clone()).get(id).await?))
}
