...
/// 构造一个物料,返回 id
pub async fn seed_material(pool: &PgPool, code: &str, category: &str, allow_preissue: bool) -> i64 {
    sqlx::query_scalar(
        r#"
...
pub async fn default_bad_wh_loc(pool: &PgPool) -> (i64, i64) {
    let wh_id: i64 = sqlx::query_scalar("select id from mdm.mdm_warehouse where wh_code = 'BAD01'")
        .fetch_one(pool)
        .await
...
pub async fn default_scrap_wh_loc(pool: &PgPool) -> (i64, i64) {
    let wh_id: i64 =
        sqlx::query_scalar("select id from mdm.mdm_warehouse where wh_code = 'SCRAP01'")
            .fetch_one(pool)
            .await
